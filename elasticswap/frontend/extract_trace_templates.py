...
def extract_trace_templates_with_evolution(
    ...
    print(f"  Filtered traces: {filtered_traces} ({filtered_traces/total_traces*100:.1f}%)")
...
def trajectory_to_dag_nodes(trajectory: RequestTrajectory, annotate: bool = True):
    """
...
def generate_claude_trace_workload(
    ...
    Returns:
        Tuple of (dags, dag_names, arrival_times) for run_dags_with_arrival_times
    """
    # Extract templates
    ...
    # Convert to DAGs with unique request IDs
    dags = []
    dag_names = []
    for traj in trajectories:
        dag = trajectory_to_dag_nodes(traj, annotate=True)
        dags.append(dag)
        dag_names.append(traj.request_id)  # Use trajectory request_id as dag_name
    ...
    return dags, dag_names, arrival_times
