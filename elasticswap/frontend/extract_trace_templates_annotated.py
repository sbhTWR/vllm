...
def trajectory_to_dag_nodes(trajectory: RequestTrajectory, 
    ...
    for turn_idx, turn in enumerate(trajectory.turns):
        ...
        if has_tools:
            # Regular case: create tool nodes from actual tools
            for tool_idx, (tool_name, tool_pred) in enumerate(zip(turn.tool_names, turn.tool_predictions)):
                ...
                # Add individual model prediction to tool node metadata (optional, for debugging)
                if tool_pred:
                    tool_node.metadata["model_prediction"] = {
                        "predicted_label": tool_pred.predicted_label,
                        "probabilities": tool_pred.probabilities
                    }
                
                # Tool depends on LLM call
                tool_node.add_input("llm_output", llm_node)
                nodes.append(tool_node)
                current_turn_tool_nodes.append(tool_node)
        else:
            ...
            dummy_tool_node = ToolCallNode(
                ...
                name=f"{trajectory.request_id}_tool_{turn.turn_idx}_dummy",
                expected_time=0.0
            )
