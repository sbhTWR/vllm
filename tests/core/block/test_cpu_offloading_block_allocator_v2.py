...
def test_swap_hints(num_cpu_blocks: int, num_gpu_blocks: int,
    ...
    unique_token_ids = list(
        range(5*(num_cpu_blocks + num_gpu_blocks) * block_size))
    ...
    gpu_token_ids4 = list(
        chunk_list(
            unique_token_ids[3*num_gpu_blocks * block_size // 2: 2 * num_gpu_blocks *
                             block_size], block_size)
    )

    # allocate requests of length 256 from here on 
    ...
    gpu_token_ids8 = list(
        chunk_list(
            unique_token_ids[5 * num_gpu_blocks * block_size: 6 * num_gpu_blocks *
                             block_size], block_size)
    )

    gpu_token_ids9 = list(
        chunk_list(
            unique_token_ids[6 * num_gpu_blocks * block_size: 7 * num_gpu_blocks *
                             block_size], block_size)
    )
    ...
    # verify swap hints 
    assert allocator._allocators[Device.GPU]._block_tracker[0].reuse_expected_time_s == 10
    assert allocator._allocators[Device.GPU]._block_tracker[num_gpu_blocks//2 + 1].reuse_expected_time_s == 1
    ...
    assert user == "test_agent"
    # print("user swapped out", user)
    # assert allocator._allocators[Device.CPU]

    # try allocating agent 1 again. 
    # agent1 should be swapped-in, agent2 should be swapped out 
    ...
    # here there should be two things:
    # 1. both blocks to swap-in and swap-out should be 128 

    # get blocks for swap 
    blocks_to_swap_out, blocks_to_swap_in = allocator.get_and_reset_swaps(0.0)
    assert len(blocks_to_swap_out) == num_gpu_blocks // 2
    assert len(blocks_to_swap_in) == num_gpu_blocks // 2

    # 2. agent 1 should have been swapped-in, and agent 2 swapped out 
    gpu_block_id = blocks_to_swap_in[0][1]
    user_gpu = gpu_allocator._block_tracker[gpu_block_id].last_accessed_by_user

    cpu_block_id = blocks_to_swap_out[0][1] + num_gpu_blocks
    user_cpu = gpu_allocator._block_tracker[cpu_block_id].last_accessed_by_user

    print("agent swap in --> %s" % user_gpu)
    print("agent swap out --> %s" % user_cpu)

    assert user_gpu == "test_agent"
    assert user_cpu == "test_agent2"

    # Now allocate more requests and free them so that they end up in the cpu
    # This is to test cpu eviction 
    # 1 * 128 
    # 4 * 256 

    # free previous requests

    # mark the blocks as computed 
    allocator.mark_blocks_as_computed([block.block_id for block in gpu_blocks])
    # free blocks 
    for block in gpu_blocks:
        allocator.free(block)
    ...
    # mark the blocks as computed 
    allocator.mark_blocks_as_computed([block.block_id for block in gpu_blocks3])
    # free blocks 
    for block in gpu_blocks3:
        allocator.free(block)
    ...
    # 1 x size 128 
    seq_group.user_id = "test_agent4"
    seq_group.hints = SequenceGroupHints(kv_reuse_expected_duration_s=20)
    ...
    # mark the blocks as computed 
    allocator.mark_blocks_as_computed([block.block_id for block in gpu_blocks4])
    # free blocks 
    for block in gpu_blocks4:
        allocator.free(block)
    ...
    # 1 x size 256
    seq_group.user_id = "test_agent5"
    seq_group.hints = SequenceGroupHints(kv_reuse_expected_duration_s=30)
    ...
    # mark the blocks as computed 
    allocator.mark_blocks_as_computed([block.block_id for block in gpu_blocks5])
    # free blocks 
    for block in gpu_blocks5:
        allocator.free(block)
    # free previous request
    ...
    # 2 x size 256
    seq_group.user_id = "test_agent6"
    seq_group.hints = SequenceGroupHints(kv_reuse_expected_duration_s=40)
    ...
    with AllocationContextManager(block_allocator=allocator, seq_group=seq_group):
        ...
        gpu_blocks6 = [
            allocator.allocate_immutable_block(prev_block=None,
                                            token_ids=token_ids,
                                            device=Device.GPU)
            for token_ids in gpu_token_ids6
        ]
    ...
    # mark the blocks as computed 
    allocator.mark_blocks_as_computed([block.block_id for block in gpu_blocks6])
    # free blocks 
    for block in gpu_blocks6:
        allocator.free(block)
    # free previous request
    ...
    # 3 x size 256
    seq_group.user_id = "test_agent7"
    seq_group.hints = SequenceGroupHints(kv_reuse_expected_duration_s=50)
    ...
    with AllocationContextManager(block_allocator=allocator, seq_group=seq_group):
        ...
        gpu_blocks7 = [
            allocator.allocate_immutable_block(prev_block=None,
                                            token_ids=token_ids,
                                            device=Device.GPU)
            for token_ids in gpu_token_ids7
        ]
    ...
    # mark the blocks as computed 
    allocator.mark_blocks_as_computed([block.block_id for block in gpu_blocks7])
    # free blocks 
    for block in gpu_blocks7:
        allocator.free(block)
    # free previous request

    assert allocator.get_num_free_blocks(Device.GPU) == num_gpu_blocks

    # the following should trigger eviction.

    # 4 x size 256
    seq_group.user_id = "test_agent8"
    seq_group.hints = SequenceGroupHints(kv_reuse_expected_duration_s=60)
    ...
    with AllocationContextManager(block_allocator=allocator, seq_group=seq_group):
        ...
        gpu_blocks = [
            allocator.allocate_immutable_block(prev_block=None,
                                            token_ids=token_ids,
                                            device=Device.GPU)
            for token_ids in gpu_token_ids8
        ]
    
    # assert if the cpu blocks have been evicted 
    # what was evicted? 
